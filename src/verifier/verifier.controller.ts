...
export class VerifierController {
  ...
  getRedirectUri(path: string) {
    const base = this.configService.get("FRONTEND_BASE_URL").replace(/\/+$/g, "")
    path = path.replace(/^\/+/g, "")
    return `${base}/${path}`
  }
  ...
  async recieveCredential(@Req() req: Request, @Query() query: SharedVpDto) {
    ...
    try {
      ...
      const verifierDto = await this.verifierService.evalVpToken(query.vp_token);
    ...
    throw new InternalServerErrorException("fooo");
  }
}
